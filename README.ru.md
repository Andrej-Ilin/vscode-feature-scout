...
Feature Scout полезен:

- solo-разработчикам, которые живут в VS Code, используют AI-агентов и не хотят
  вручную читать каждый релиз VS Code;
- командам с `AGENTS.md`, `CLAUDE.md`, MCP, custom prompts или custom agents,
  которые хотят, чтобы возможности редактора попадали в контекст агентов;
- maintainers, которым нужен маленький повторяемый отчет вместо надежды на то,
  что кто-то случайно заметит новую полезную возможность VS Code.
...
## Типичный Агентный Цикл

Задуманный цикл специально простой:

1. VS Code стартует, перезапускается или обновляется.
2. Опциональный LaunchAgent запускает Feature Scout.
3. Feature Scout пишет `latest.md` или `baseline.md`.
4. Codex, Claude Code, Copilot, Continue или другой агент читает этот отчет как
   компактный контекст.
5. Агент адаптирует project instructions, prompts, tasks или MCP configuration
   только когда отчет показывает полезную и релевантную возможность.

## Пример Отчета

Укороченный фрагмент baseline report:
...
Укороченный фрагмент release report:
...
## Почему Не Просто Читать Release Notes?

Можно и так. Feature Scout нужен, когда обычного чтения release notes
недостаточно:

- release notes глобальные, а Feature Scout фильтрует их через профиль вашего
  проекта;
- release notes не знают, есть ли в вашем репозитории `AGENTS.md`,
  `.vscode/mcp.json`, `.vscode/tasks.json` или custom agents;
- `state.json` позволяет автоматизации пропускать уже обработанные релизы;
- baseline mode находит старые возможности VS Code, которые уже не считаются
  "новыми", но все еще могут быть полезны;
- результат сразу оформлен как маленький Markdown-handoff для AI-агентов.
...
Конкретные сигналы репозитория специально скучные и явные. Baseline mode
проверяет пути вроде:
